              ...
              {assets.map((a) => (
                <tr key={a.id}>
                  ...
                  <td className="px-4 py-2 text-right">
                    ...
                    {a.status === "ASSIGNED" && (
                      <button onClick={() => returnAsset(a.id)} className="text-xs font-medium text-indigo-600 hover:underline">
                        Return
                      </button>
                    )}
                    {a.status !== "RETIRED" && (
...
function AssetForm({ onCreated }: { onCreated: () => void }) {
  ...
  return (
    <form onSubmit={onSubmit} className="grid grid-cols-1 gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-3">
      <input required placeholder="Asset name" value={name} onChange={(e) => setName(e.target.value)} className="rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" />
      ...
      <input placeholder="Serial number (optional)" value={serialNumber} onChange={(e) => setSerialNumber(e.target.value)} className="rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" />
