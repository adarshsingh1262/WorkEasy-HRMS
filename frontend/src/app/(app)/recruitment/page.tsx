        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              postings.map((p) => (
                <div
                  ...
                  className={`block w-full cursor-pointer rounded-lg border p-4 text-left shadow-sm transition ${
                    selectedPostingId === p.id ? "border-slate-900" : "border-slate-200 hover:border-slate-400"
                  }`}
                >
                  <div className="flex items-center justify-between">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              {selectedPosting && (
                <button onClick={() => setShowCandidateForm((v) => !v)} className="text-xs font-medium text-slate-500 hover:underline">
...
function PostingForm({ onCreated }: { onCreated: () => void }) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <input required placeholder="Job title" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
      {departments.length > 0 && (
        <select value={departmentId} onChange={(e) => setDepartmentId(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
      ...
      <textarea required placeholder="Job description" value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
    ...
    <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      ...
      {candidate.stage !== "HIRED" && (
        <div className="mt-3 flex flex-wrap gap-2">
          <button onClick={() => setShowInterviewForm((v) => !v)} className="text-xs font-medium text-slate-500 hover:underline">
            Schedule interview
          </button>
...
function InterviewForm({
  ...
  return (
    <form onSubmit={onSubmit} className="mt-2 grid grid-cols-2 gap-2 rounded-md bg-slate-50 p-2 text-xs">
      ...
      <button type="submit" disabled={submitting} className="col-span-2 w-fit rounded-md bg-slate-900 px-3 py-1 font-medium text-white disabled:opacity-50">
...
function HireForm({ candidateId, onHired }: { candidateId: string; onHired: () => void }) {
  ...
  return (
    <form onSubmit={onSubmit} className="mt-2 space-y-2 rounded-md bg-slate-50 p-2 text-xs">
      ...
      <button type="submit" disabled={submitting} className="w-fit rounded-md bg-slate-900 px-3 py-1 font-medium text-white disabled:opacity-50">
