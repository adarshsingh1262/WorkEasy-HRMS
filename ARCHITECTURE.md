...
## 4b. Additional Modules (to match Zoho People / Razorpay X / Darwinbox / Keka feature parity)

Your original nav covers the "core HR suite." Leading HRMS products also ship these as first-class modules — adding them to the nav and roadmap:
...
- **Mobile App** — React Native (shares `packages/shared-types`) for attendance check-in/out (with geofencing), leave apply/approve, payslip view, push notifications.
